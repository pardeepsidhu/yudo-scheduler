...
const FeatureCard = ({ icon: Icon, title, desc, accent }: any) => (
  <div className="group flex items-start gap-4 rounded-2xl border border-white/60 bg-white/50 p-5 backdrop-blur transition-all duration-300 hover:bg-white/80 hover:shadow-md hover:shadow-[#3A41E5]/20 cursor-pointer">
    <div className={`shrink-0 rounded-xl p-2.5 ${accent}`}>
      <Icon className="h-5 w-5 text-[#3A41E5]" />
    </div>
    <div>
      <p className="text-sm font-semibold text-[#1F257A]">{title}</p>
      <p className="mt-0.5 text-xs leading-relaxed text-[#1F257A]/80">{desc}</p>
    </div>
  </div>
);
...
const Field = ({ label, icon: Icon, children, hint }: any) => (
  <div>
    ...
    <div className="flex items-center gap-3 rounded-xl border border-[#3A41E5]/20 bg-white px-4 py-3 text-[#1F257A] transition-all focus-within:border-[#3A41E5] focus-within:ring-2 focus-within:ring-[#3A41E5]/10">
      <Icon className="h-4 w-4 shrink-0 text-[#3A41E5]/70" />
      {children}
    </div>
...
const OtpRow = ({ otp, handleOtpChange }: any) => (
  <div className="flex gap-3">
    {(otp ?? ["", "", "", ""]).map((digit: string, i: number) => (
      <div
        key={i}
        className="flex-1 rounded-xl border-2 border-[#3A41E5]/15 bg-white transition-colors focus-within:border-[#3A41E5]"
      >
        <input
          id={`otp-${i}`}
          maxLength={1}
          ...
          className="w-full h-12 text-center text-xl font-bold bg-transparent outline-none text-[#1F257A]"
        />
      </div>
    ))}
  </div>
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden font-sans">
      <div className="relative flex w-full flex-col overflow-hidden bg-slate-50 md:w-[480px] md:shrink-0 p-1">
        <div className="relative h-full w-full border border-[#3A41E5]/25 bg-white/50 p-[6px] overflow-hidden">
          <div className="relative h-full w-full border border-[#3A41E5]/20 bg-[#F7F8FF] overflow-y-auto">
            <div
            ...
            <div className="relative z-10 w-full max-w-sm mx-auto px-6 py-6">
              ...
              {resetId ? (
                <div className="space-y-5">
                  <div>
                    <h2 className="text-2xl font-bold text-[#111827]">Set new password</h2>
                  ...
                  <form
                    ...
                    onSubmit={async (e) => {
                      e.preventDefault();
                      await handleResetPassword(setWaiting, password, toast, navigate,resetId);
                    }}
                  >
                    ...
                    <Button type="submit" disabled={waiting} className="w-full mt-2 bg-[#3A41E5] hover:bg-[#2F36C7] text-white">
                  ...
                  <div className="text-center">
                    <Button variant="link" className="text-[#3A41E5]" onClick={() => (window.location.href = window.location.pathname)}>
                      ← Back to login
                    </Button>
                  </div>
                </div>
                ...
                <div className="space-y-5">
                  <div className="flex items-center gap-3 shrink-0">
                    <div className="flex h-12 w-12 items-center justify-center rounded-[4px] bg-[linear-gradient(135deg,_#3A41E5_0%,_#4F56F0_50%,_#636BFF_100%)] backdrop-blur-sm">
                      <Calendar className="h-5 w-5 text-white" />
                    </div>
                    <div>
                      <h2 className="text-2xl font-bold text-[#1F257A]">
                        {tab === "login" ? "Welcome back" : "Create account"}
                      </h2>
                      <p className="mt-1 text-sm text-[#1F257A]/75">
                  ...
                  <div className="flex rounded-xl bg-[#EEF0FF] p-1">
                    {["login", "signup"].map((t) => (
                      <button
                        key={t}
                        type="button"
                        ...
                        ${tab === t ? "bg-white text-[#1F257A] shadow-sm" : "text-[#3A41E5]/70 hover:text-[#3A41E5]"}`}
                      >
                        {t}
                      </button>
                    ))}
                  ...
                  {tab === "login" && (
                    <form className="space-y-4" onSubmit={handleLogin}>
                      ...
                      <button
                        ...
                        className="flex items-center gap-1.5 text-xs font-medium text-[#3A41E5] hover:text-[#2F36C7] transition-colors"
                      >
                      ...
                      <Button type="submit" disabled={waiting} className="yudo-btn bg-[#3A41E5] hover:bg-[#2F36C7] text-white">
                        {waiting
                  ...
                  {tab === "signup" && (
                    <form className="space-y-4" onSubmit={handleVerifyOtp}>
                      ...
                      {otpSent && (
                        <div className="space-y-3 rounded-2xl border border-[#3A41E5]/15 bg-[#EEF0FF]/80 p-4">
                          <div>
                            <p className="text-xs font-semibold uppercase tracking-widest text-[#3A41E5]">Verification code</p>
                          ...
                          <div className="flex items-center justify-between">
                            <button
                              ...
                              disabled={otpTimer > 0}
                              className="text-xs font-medium text-[#3A41E5] hover:text-[#2F36C7] disabled:text-slate-400 transition-colors"
                            >
                      ...
                      <div className="flex gap-2 pt-1">
                        {!otpSent ? (
                          <>
                            <div className="w-1/2">
                              <Button
                                ...
                                onClick={handleSendOtp}
                                disabled={waiting}
                                className="yudo-btn w-full bg-[#3A41E5] hover:bg-[#2F36C7] text-white"
                              >
                                {waiting ? (
                                  <>
                          ...
                          <Button
                            ...
                            className="w-full bg-[#3A41E5] hover:bg-[#2F36C7] text-white"
                          >
                            {waiting ? (
                              <>
                                <Loader2 className="mr-2 h-4 w-4 animate-spin" />
      ...
      <div className="relative hidden flex-1 flex-col overflow-hidden bg-gradient-to-br from-[#3A41E5] via-[#4F56F0] to-[#636BFF] p-8 md:flex lg:p-12">
        <div className="pointer-events-none absolute -top-24 -right-24 h-96 w-96 rounded-full bg-white/10" />
        <div className="pointer-events-none absolute -bottom-32 -left-16 h-80 w-80 rounded-full bg-[#8E93FF]/20" />
        <div className="pointer-events-none absolute top-1/2 right-0 h-56 w-56 -translate-y-1/2 rounded-full bg-[#AAB0FF]/20" />
        ...
        <div className="relative z-10 flex flex-col h-full justify-between min-h-0">
          ...
          <div className="space-y-6 shrink-0">
            <div className="space-y-3">
              ...
              <p className="max-w-md text-sm leading-relaxed text-indigo-100 lg:text-base">
            ...
            <div className="flex -space-x-2">
              {[
                ...
                <img
                  ...
                  src={src}
                  alt="user avatar"
                  className="h-8 w-8 rounded-full border-2 border-[#4F56F0] object-cover"
                />
              ))}
            </div>
